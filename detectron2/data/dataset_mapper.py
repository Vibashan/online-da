...
class SW_DatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self.is_train               = is_train
        self.augmentations          = T.AugmentationList(augmentations)
        self.no_flip_aug            = T.AugmentationList([T.ResizeShortestEdge(cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST, "choice")])
        self.image_format           = image_format
        self.use_instance_mask      = use_instance_mask
        self.instance_mask_format   = instance_mask_format
        self.use_keypoint           = use_keypoint
    ...
    def __call__(self, dataset_dict):
        ...
        #no_flip_transforms = self.no_flip_aug(aug_input)
        #image_weak_aug, _ = aug_input.image, aug_input.sem_seg

        image_shape = image_weak_aug.shape[:2]  # h, w
        # Pytorch's dataloader is efficient on torch.Tensor due to shared-memory,
        # but not efficient on large generic data structures due to the use of pickle & mp.Queue.
        # Therefore it's important to use torch.Tensor.
        dataset_dict["image"] = torch.as_tensor(np.ascontiguousarray(image_weak_aug.transpose(2, 0, 1)))
