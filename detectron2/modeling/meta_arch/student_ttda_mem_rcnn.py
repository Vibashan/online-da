...
from .losses import MemConLoss_trans
from .memory_bank import Memory_trans_update, Memory_trans_read

__all__ = ["student_ttda_mem_RCNN"]

@META_ARCH_REGISTRY.register()
class student_ttda_mem_RCNN(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        mem_items = 1024    
        self.mem_bank = F.normalize(torch.rand((mem_items, 2048), dtype=torch.float), dim=1).cuda()

        self.memory_update = Memory_trans_update(memory_size=mem_items, feature_dim=2048, key_dim=2048, temp_update=0.1, temp_gather=0.1)
        self.memory_read = Memory_trans_read()
        self.mem_contrastive_loss = MemConLoss_trans(temperature=0.07) 
    ...
    # KL divergence loss
    def dist_loss(self, student_logits, teacher_logits) :
        teacher_prob = F.softmax(teacher_logits, dim=1)
        student_log_prob = F.log_softmax(student_logits, dim=1)
        dist_loss = F.kl_div(student_log_prob, teacher_prob.detach(), reduction='batchmean')
        return dist_loss

    # Knowledge distillation loss {input - encoder features}
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], cfg=None, model_teacher=None, 
                t_features=None, t_proposals=None, t_results=None, t_mem_results=None, mode="test"):
        """
        Args:
        ...
        s_query = self.query_head(s_box_features.mean(dim=[2, 3]))
        s_value = self.value_head(s_box_features.mean(dim=[2, 3]))
        t_query = self.query_head(t_box_features.mean(dim=[2, 3]))
        t_value = self.value_head(t_box_features.mean(dim=[2, 3]))

        self.mem_bank = self.memory_update(self.mem_bank,
                                            t_query.contiguous().unsqueeze(-1).unsqueeze(-1), 
                                            t_value.contiguous().unsqueeze(-1).unsqueeze(-1),
                                            )

        mem_s_query  = self.memory_read(self.mem_bank,
                                            s_query.contiguous().unsqueeze(-1).unsqueeze(-1), 
                                            s_value.contiguous().unsqueeze(-1).unsqueeze(-1),
                                            )

        losses["MemCLR"] = self.mem_contrastive_loss(s_query, s_box_features, mem_s_query.squeeze(-1).squeeze(-1), s_value, t_box_features, t_value, self.mem_bank)     
        losses["st_const"] = self.KD_loss(s_roih_logits[0], t_roih_logits[0])                  

        return losses

    def mem_bank_control(self, t_pooled_feat, obj_cls, max_items):

        if len(self.mem_bank[obj_cls]) > 0:
            self.mem_bank[obj_cls] = [torch.cat([self.mem_bank[obj_cls][0], t_pooled_feat],0)]
            if len(self.mem_bank[obj_cls][0]) > max_items:
                remove_len = len(self.mem_bank[obj_cls][0]) - max_items
                self.mem_bank[obj_cls] = [self.mem_bank[obj_cls][0][remove_len:]]
        else:
            self.mem_bank[obj_cls].append(t_pooled_feat)
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return student_ttda_mem_RCNN._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
